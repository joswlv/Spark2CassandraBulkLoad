...
case class SparkCassWriteConf(
  ...
  private[cassandra] val optionsAsColumns
    : (String, String) => Seq[ColumnDef] = { (keyspace, table) =>
    ...
    Seq(toRegularColDef(ttl, DataType.cint()),
...
object SparkCassWriteConf {
  ...
  val SPARK_CASSANDRA_BULK_WRITE_THROUGHPUT_MB_PER_SEC =
    SparkCassConfParam[Int](
